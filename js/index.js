...
function generate(event) {
    ...
    let npc_dict = create_npc(name, cr, race, [weapon], type, hp_option, damage_option);
    ...
    document.querySelector("#attack_name").innerText = `${npc_dict["weapons"][0]} x${npc_dict["num_attacks"]}`;
    document.querySelector("#attack_bonus").innerText = mod(npc_dict["attack"]);
    document.querySelector("#damage").innerText = Math.round(npc_dict["total_damage"] / npc_dict["num_attacks"]);
